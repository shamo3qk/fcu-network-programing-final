...
class BulletManager:
    def __init__(self, total_bullets, live_bullets):
        self.total_bullets = total_bullets  # 總子彈數
        self.live_bullets = live_bullets  # 本輪實彈數
        self.remaining_live_bullets = live_bullets  # 剩餘實彈數
        self.bullet_positions = []  # 實彈位置
        self.initialize_bullets()  # 初始化實彈位置

    def initialize_bullets(self):
        """隨機選擇實彈位置"""
        self.bullet_positions = random.sample(range(self.total_bullets), self.live_bullets)
        self.remaining_live_bullets = self.live_bullets

    def update_positions(self):
        """更新彈匣內每顆子彈的位置"""
        self.bullet_positions = [pos - 1 for pos in self.bullet_positions if pos > 0]

    def shoot(self):
        """模擬射擊，檢查是否射出實彈"""
        if 0 in self.bullet_positions:
            self.remaining_live_bullets -= 1
            self.bullet_positions.remove(0)  # 移除已射出的實彈
            if self.remaining_live_bullets == 0:
                self.initialize_bullets()  # 剩餘實彈歸零時重新隨機分配
            return True  # 發射實彈
        return False  # 未發射實彈
...
def game_loop(player1, player2, player1_life, player2_life, bullet_manager):
    ...
    while player1_life > 0 and player2_life > 0:
        ...
        opponent_player[0].send(f"Game start! Your life: {opponent_life}".encode('utf-8')) 
        time.sleep(1)
